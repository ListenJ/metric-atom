...
class Atom3D(BaseAtom):
    ...
    def __init__(self, mu, radius, color, feature_dim=16, eps=0.5):
        ...
        if mu.dim() != 1 or mu.shape[0] != 3:
            raise ValueError(f"mu must be (3,), got {mu.shape}")
    ...
    def forward(self, x, metric_fn):
        ...
        Args:
            x: (N, 3) 查询点
            metric_fn: 返回 g(x) 的函数，需返回 (N, 3, 3) 或 (1, 3, 3)
        ...
        # 在原子中心处采样度量
        g = metric_fn(self._mu.unsqueeze(0))  # (1, 3, 3)

        # 位移向量
        dx = x - self._mu.unsqueeze(0)  # (N, 3)

        # 马氏距离平方: dx^T g dx
        # g: (1, 3, 3), dx: (N, 3)
        gx = torch.matmul(dx.unsqueeze(1), g).squeeze(1)  # (N, 3)
