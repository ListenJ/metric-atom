...
from src.losses.occupancy_coupling import occupancy_coupling_loss_mlp
...
def seed_atoms_smart(atoms, pred_color, target_img, H, W, device,
    ...
    print(f"  [Seed] +{len(new_atoms)} (score-based, μ_score={score.max():.3f})")
...
def train_scene_mlp(H=64, W=64, num_atoms=200, num_epochs=1000, num_views=8,
                    phase2_start=400, lr=1e-3, device='cpu', output_dir='outputs/2d_mlp'):
    ...
    print(f"[2/5] 初始化 MLP度量场 + {num_atoms} 个原子...")
    metric_field = MLPMetricField2D(hidden_dim=128, num_layers=4, init_scale=1.0).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        loss_vol = occupancy_coupling_loss_mlp(metric_field, occupancy, H, W,
                                                g_occ_target=1.0, g_bg_target=10.0) * w_vol
        loss = loss_render + loss_vol
...
if __name__ == '__main__':
    ...
    atoms, field, log, metrics = train_scene_mlp(
        H=64, W=64,
        num_atoms=200,
        num_epochs=1000,
        num_views=8,
