...
建议策略：$\varepsilon_t = \varepsilon_{min} + (\varepsilon_{max} - \varepsilon_{min}) \cdot \exp(-t / \tau_{cool})$

例如：$\varepsilon_{max} = 0.15, \varepsilon_{min} = 0.02, \tau_{cool} = 200$
...
2. **收敛速率：** 梯度下降下 L_direct 的收敛速率是多少？O(1/t)？线性？
