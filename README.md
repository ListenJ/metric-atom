...
| InfoNCE 对比聚类 | ✅ 突破确认 | ARI 0.006→0.175（29×提升），见下方训练报告 |
| 论文撰写 | 📝 初稿中 | 私有仓库 |

### 🏆 最新成果：无监督聚类突破（2026-05）

在 2D 场景验证中实现了**对比学习驱动的无监督聚类突破**：

| 指标 | 突破前 | 突破后 | 提升 |
|---|---|---|---|
| **ARI**（调整兰德指数） | 0.006 | **0.175** | **29×** |
| **凝聚损失** | -22.178（发散） | **+0.232**（稳定） | ✅ 有界收敛 |
| **总损失** | -21.14（负值主导） | **+4.909**（正值） | ✅ 正常训练 |
| **占位迹** | 0.541（模糊） | **4.201**（明确分离） | ✅ 物景区分 |

**三项技术贡献**（`feat/clustering-breakthrough` 分支）：
1. **KMeans 空间特征初始化**：Phase 1→2 时用原子位置 K=2 聚类，为每个簇分配不同的初始特征向量，打破对称性
2. **InfoNCE 对比损失**：替换原吸引/方差排斥损失，基于测地距离定义正负样本对，损失天然有界于 [0, log(N)]
3. **度量耦合增强**：E_vol 权重 0.02→0.2，迫使度量场学习更锐利的物体/背景边界

### 📋 近期计划

- 覆盖率达标后：使用测地邻接改进特征-空间耦合
- 在真实图像上验证度量场分离能力
- 多物体 3D 场景聚类评估（当前为 2 物体）
- ARI 突破 0.5 目标：超参扫描（w_coh、tau、pos_thresh 组合优化）
- 时间维度扩展 g(x,t)
